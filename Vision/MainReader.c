 ...
 * -------------------------------------------------------------
 */
vision_void bErrorPrint(vision_char* fmt, ...);
vision_void displayBuffer(BufferReader* ptr_Buffer);
vision_long getFileSize(vision_char* fname);
vision_intg isNumber(const vision_char* ns);
vision_void startReader(vision_char*, vision_char*, vision_char, vision_intg, vision_intg);
...
vision_intg mainReader(vision_intg argc, vision_char** argv) {
	...
	vision_char* program = argv[0];
	vision_char* input = argv[2];
	vision_char mode = MODE_FIXED;
	vision_intg size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
vision_void startReader(vision_char* program, vision_char* input, vision_char mode, vision_intg size, vision_intg increment) {

	ReaderPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	vision_intg loadSize = 0;		/* the size of the file loaded in the buffer */
	vision_char symbol;			/* symbol read from input file */
