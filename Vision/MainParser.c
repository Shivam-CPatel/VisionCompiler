...
vision_intg errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern vision_intg syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern vision_intg line; /* source code line number - defined in scanner.c */
...
extern vision_void startParser(vision_void);
extern vision_intg startScanner(ReaderPointer sc_buf);

static vision_void printParserError(vision_char* fmt, ...);
static vision_void displayParser(ReaderPointer ptrBuffer);
static vision_long getParserFileSize(vision_char* fname);
static vision_void callGarbageCollector(vision_void);
...
vision_intg mainParser(vision_intg argc, vision_char** argv) {
	...
	vision_intg loadsize = 0; /*the size of the file loaded in the buffer */
...
vision_void printParserError(vision_char* fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(vision_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
vision_long getParserFileSize(vision_char* fname) {
	FILE* input;
	vision_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
vision_void displayParser(ReaderPointer ptrBuffer) {
...
vision_void callGarbageCollector(boa_void) {
